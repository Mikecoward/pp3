...
public class Axx {
    ...
    public Axx(CRServo servo, AnalogInput encoder) {
        rtp = true;
        this.servo = servo;
    ...
    public Axx(CRServo servo, AnalogInput encoder, Direction direction) {
        this(servo, encoder);
    ...
    public static class CRAxonTest extends LinearOpMode {
        ...
        public void runOpMode() throws InterruptedException {
            // Combine dashboard and normal telemetry

            // Hardware mapping
            CRServo crservo = hardwareMap.crservo.get("spinner");
            AnalogInput encoder = hardwareMap.get(AnalogInput.class, "spinEncoder");

            // PID-controlled servo
            Axx servo = new Axx(crservo, encoder);

            waitForStart();

            while (!isStopRequested()) {
                // ----- Manual controls using only gamepad1 -----

                // Change target rotation
                if (gamepad1.dpad_up) {
                ...
                // Reset PID
                if (gamepad1.guide) { // touchpad equivalent
                    servo.setKP(0.015);
                    servo.setKI(0.0005);
                    servo.setKD(0.0025);
                    servo.resetPID();
